let appData = [];
    ...
    var SESSION_CACHE_KEY = 'pk2_initial_v5';
    ...
    window.addEventListener('pk2-prefetch-ready', function (ev) {
      if (!ev.detail || !ev.detail.apps) return;
      window.__PK2_PENDING_DATA__ = ev.detail;
      if (bootFinished) {
        initApp(ev.detail, { fromCache: false, silent: true });
      }
    });

    function clearBootPrefetchGlobals() {
      try { delete window.__BOOT_DATA__; } catch (e) {}
      try { delete window.__PREFETCH_DATA__; } catch (e) {}
    }

    function invalidateSessionCache() {
      memoryCachePayload = null;
      clearBootPrefetchGlobals();
    ...
    function applySiteBranding(data) {
      ...
      var iconEl = document.getElementById('default-logo-icon');
      if (data.logoImage) {
        if (logoEl) {
      ...
      if (data.bgImage) {
        document.body.style.backgroundImage = "url('" + String(data.bgImage).replace(/'/g, "\\'") + "')";
      } else {
    ...
    function persistCache(data) {
      var envelope = { t: Date.now(), data: data };
      memoryCachePayload = envelope;
    ...
    function readCachedPayload() {
      ...
      if (memoryCachePayload && memoryCachePayload.data) return memoryCachePayload;
      try {
    ...
    function loadInitialData(success, failure, opts) {
      opts = opts || {};
      if (!opts.forceFresh) {
        if (window.__PREFETCH_DATA__ && window.__PREFETCH_DATA__.apps) {
    ...
    function scheduleReorderSave() {
      ...
      reorderSaveTimer = setTimeout(function () {
        ...
        reorderSaving = true;
        var orderedIds = sortAppsForDisplay(appData, 'order').map(function (a) { return a.id; });
        gasRun()
          .reorderApps(orderedIds)
          .withSuccessHandler(function (apps) {
            reorderSaving = false;
            appData = apps || appData;
            var cached = readCachedPayload();
            if (cached && cached.data) {
              cached.data.apps = appData;
              persistCache(cached.data);
            } else {
              persistCache({ apps: appData, email: document.getElementById('user-email').innerText || '' });
            }
            renderApps();
          })
          ...
          '<div class="lux-app-card ' + cardClass + sortableClass + '"' + dragAttrs + clickHandler + '>' +
            ...
            '<article class="app-card-shell">' +
              '<div class="app-card-visual">' +
                '<img class="app-card-thumb" src="' + escHtml(imgUrl) + '" alt="' + escHtml(app.name) + '" width="128" height="128" loading="' + imgLoading + '" decoding="async" fetchpriority="' + imgPriority + '" />' +
              '</div>' +
    ...
    function confirmCrop() {
      ...
      const maxSide = isBranding ? 560 : 320;
      const quality = isBranding ? 0.8 : 0.68;
    ...
    function logout() {
      isAdmin = false;
      editingAppId = null;
      try { sessionStorage.removeItem('pk2_admin_session'); } catch (e) {}
